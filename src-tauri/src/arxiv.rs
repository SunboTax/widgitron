...
const MIN_ARXIV_POLL_INTERVAL_SECS: u64 = 60;
const MAX_ARXIV_POLL_INTERVAL_SECS: u64 = 86_400;

pub fn normalize_arxiv_config(config: &mut ArxivConfig) {
    config.update_interval = clamp_poll_interval_secs(
        config.update_interval,
        MIN_ARXIV_POLL_INTERVAL_SECS,
        MAX_ARXIV_POLL_INTERVAL_SECS,
    );
}
...
pub async fn start_arxiv_monitor(app: AppHandle, state: std::sync::Arc<GlobalState>) {
    ...
    loop {
        ...
        normalize_arxiv_config(&mut config);
